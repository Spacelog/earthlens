...
def series_queryset(series):
    if series == "index":
        return Image.objects.exclude(group_hides=True).order_by("-rating", "-votes", "date").distinct()
    elif series.startswith("m-"):
        return Image.objects.exclude(group_hides=True).filter(
                mission__code__iexact=series[2:]).order_by("-rating", "-votes", "date").distinct()
        ...
        return Image.objects.exclude(group_hides=True).filter(
                tag_objects__tagged__slug=series[2:]).order_by("-rating", "-votes", "date").distinct()
        ...
        return Image.objects.filter(vote_objects__user__username=series[3:],
                                    vote_objects__vote=3).order_by("date", "code").distinct()
    else:
